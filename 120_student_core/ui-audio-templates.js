...
window.UIAudioTemplates = {
    getRecordingStudioHTML: function(transcriptText, taskTitle, materialUrl, materialRange) {
        ...
        if (materialUrl && materialUrl.trim() !== '') {
            ...
            // 🌟 完全對齊圖三的樣式：按鈕左對齊，文字與按鈕垂直置中
            materialLinkHtml = `
                <div style="margin-bottom: 15px; padding: 12px 16px; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; display: flex; align-items: center; flex-wrap: wrap; gap: 10px;">
                    <span style="font-weight: 800; color: #166534; font-size: 0.95rem; display: flex; align-items: center;">📂 參考教材：</span>
                    <a href="${materialUrl}" target="_blank" style="background: #3b82f6; color: white; padding: 6px 14px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 0.9rem; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 1px 2px rgba(59,130,246,0.3); transition: 0.2s;" onmouseover="this.style.backgroundColor='#2563eb'" onmouseout="this.style.backgroundColor='#3b82f6'">
                        📄 開啟教材原檔
                    </a>
                    ${rangeText}
                </div>
