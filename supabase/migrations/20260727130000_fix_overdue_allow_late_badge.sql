-- 修正過期訊息「一律缺交」：
-- 1) 掃描時 allow_late 改為：作業 late_policy → 作業頂層 allow_late → 班級預設
-- 2) 回填既有 overdue 訊息的 payload.allow_late
...
BEGIN
  ...
  LOOP
    ...
    v_class_allow_late := COALESCE(
      (a_rec.class_raw->'late_submission_defaults'->>'allow_late')::boolean,
      false
    );

    -- 作業有 late_policy 用作業；否則頂層 allow_late；再否則班級預設
    IF a_rec.raw_data ? 'late_policy'
       AND jsonb_typeof(a_rec.raw_data->'late_policy') = 'object' THEN
      v_allow_late := COALESCE(
        (a_rec.raw_data->'late_policy'->>'allow_late')::boolean,
        false
      );
    ELSIF a_rec.raw_data ? 'allow_late' THEN
      v_allow_late := COALESCE((a_rec.raw_data->>'allow_late')::boolean, false);
    ELSE
      v_allow_late := v_class_allow_late;
    END IF;
...
-- 回填既有過期訊息的 allow_late（舊列常缺此欄，前端才會一律顯示缺交）
UPDATE public.user_notifications n
SET payload = COALESCE(n.payload, '{}'::jsonb) || jsonb_build_object(
  'allow_late',
  CASE
    WHEN a.raw_data ? 'late_policy'
         AND jsonb_typeof(a.raw_data->'late_policy') = 'object'
      THEN COALESCE((a.raw_data->'late_policy'->>'allow_late')::boolean, false)
    WHEN a.raw_data ? 'allow_late'
      THEN COALESCE((a.raw_data->>'allow_late')::boolean, false)
    ELSE COALESCE(
      (c.raw_data->'late_submission_defaults'->>'allow_late')::boolean,
      false
    )
  END
)
