-- 為 profiles.raw_data 增加 emailSecondary：用於學生 Drive 授權（避免 .edu 登入跳出申請存取）
...
BEGIN
  ...
  new_raw := old_raw
    || jsonb_build_object(
      ...
      -- Drive 授權用的第二 Email（不影響登入驗證；登入仍以 profiles.email 為準）
      'emailSecondary', COALESCE(p_email_secondary, '')
    );
...
GRANT EXECUTE ON FUNCTION public.staff_update_member_profile(
  uuid, uuid, text,
