...
window.FeatureMaterialBook = (function () {
    ...
    /** 口說／書寫那一格的標籤＝上面那一列範圍（主／次、頁碼、標題、大題…）。沒填＝區段 N。不准借口說當目錄文稿。 */
    ...
    function onBookPickChange(sel, pathStr) {
        ...
        if (window.FeatureTimeline && typeof window.FeatureTimeline.onRangePackChange === 'function') {
            window.FeatureTimeline.onRangePackChange(pathStr, { rerender: true, skipSync: true });
        }
    }
    ...
    function catalogRows(combo) {
        ...
        var folder = folderKey(combo && combo.folder_name);
        (_hintsByFolder[folder] || []).forEach(add);
        return out;
    }

    function rangeCatalog(comboId) {
        var combo = _combos.filter(function (c) { return String(c.id) === String(comboId || ''); })[0]
            || { id: comboId, folder_name: '' };
        var items = catalogRows(combo);
        ...
        function matchHeading(it, primary, secondaryUnit, heading) {
            return matchUnit(it, primary, secondaryUnit) && sameText(it.heading, heading);
        }
        function matchPage(it, page) {
            if (!trim(page)) return true;
            return sameText(it.page, page);
        }
        return {
            ...
            headingsOf: function (primary, secondaryUnit) {
                return uniqueMenuValues(items.filter(function (it) {
                    return matchUnit(it, primary, secondaryUnit);
            ...
            majorsOf: function (primary, secondaryUnit, heading, page) {
                return uniqueMenuValues(items.filter(function (it) {
                    return matchHeading(it, primary, secondaryUnit, heading) && matchPage(it, page);
                }).map(function (it) { return it.major; }));
            },
            secondariesOf: function (primary, secondaryUnit, heading, major) {
                var m = trim(major).toUpperCase();
                return uniqueMenuValues(items.filter(function (it) {
                    return matchHeading(it, primary, secondaryUnit, heading)
                        && trim(it.major).toUpperCase() === m;
                }).map(function (it) { return it.secondary; }));
            },
            minorsOf: function (primary, secondaryUnit, heading, major, secondary) {
                var m = trim(major).toUpperCase();
    ...
    async function loadCollectorHints() {
        ...
        _combos.forEach(function (c) {
            var folder = folderKey(c.folder_name);
            var named = folderKey(c.label);
            if (folder) want[folder] = true;
            if (named) want[named] = true;
        });
        if (!Object.keys(want).length) return;
        var blockRes = await window.supabaseClient.from('class_script_blocks').select('id, label');
        if (blockRes.error) {
            console.warn('[FeatureMaterialBook] 讀已有範圍（收集文稿）失敗', blockRes.error);
            return;
        }
        var folderOf = {};
        var ids = [];
        (blockRes.data || []).forEach(function (b) {
            var folder = folderKey(b && b.label);
            if (!folder || !want[folder]) return;
            folderOf[String(b.id)] = folder;
            ids.push(b.id);
        });
        if (!ids.length) return;
        var itemRes = await window.supabaseClient
            .from('class_script_block_items')
            .select('block_id, snapshot')
            .in('block_id', ids);
        if (itemRes.error) {
        ...
        (itemRes.data || []).forEach(function (it) {
            var folder = folderOf[String(it.block_id)];
            if (!folder) return;
            var segs = (it.snapshot && Array.isArray(it.snapshot.segments)) ? it.snapshot.segments : [];
            if (!_hintsByFolder[folder]) _hintsByFolder[folder] = [];
            segs.forEach(function (seg) {
                _hintsByFolder[folder].push({
    ...
    function loadRangeItemsInBackground() {
        if (!window.supabaseClient) return;
        var itemSelect = 'id, book_combo_id, teacher_id, primary_unit, secondary_unit, heading, major, secondary, minor, script, source_assignment_id, source_task_id, progress_date, drive_file_id, drive_file_name, updated_at';
        window.supabaseClient
            ...
            .then(function (itemRes) {
                if (itemRes.error) {
                    console.warn('[FeatureMaterialBook] 範圍列載入失敗（目錄套餐卡仍顯示）', itemRes.error);
                    _items = [];
                    return;
                }
                _items = itemRes.data || [];
    ...
    function treeHtml(combo) {
        ...
        var primaries = uniqueValues(items.map(function (it) { return it.primary_unit || '（未填主單元）'; }));
        return primaries.map(function (pu) {
                    ...
                    var majHtml = majors.map(function (maj) {
                        ...
                        var secHtml = secs.map(function (sec) {
                            ...
                            var leaves = secItems.map(function (it) {
                                ...
                                return '<div style="margin:2px 0 2px 42px; font-size:0.76rem; color:#334155;">▫️ '
                                    + esc(lab) + (hasScript ? '　有文稿' : '　文稿空') + esc(file) + '</div>';
                            }).join('');
                            return '<div style="margin:4px 0 2px 28px; font-weight:800; color:#334155; font-size:0.8rem;">次題　'
                                + esc(sec) + '</div>' + leaves;
                        }).join('');
                        return '<div style="margin:6px 0 2px 22px; font-weight:800; color:#1E3A8A; font-size:0.82rem;">大題　'
                            + esc(maj) + '</div>' + secHtml;
                    }).join('');
                    return '<div style="margin:6px 0 2px 16px; font-weight:800; color:#334155; font-size:0.82rem;">標題　'
                        + esc(hd) + '</div>' + majHtml;
                }).join('');
                return '<div style="margin:6px 0 2px 10px; font-weight:800; color:#334155; font-size:0.84rem;">次單元　'
                    + esc(su) + '</div>' + headHtml;
            }).join('');
            return '<div style="margin-top:8px;"><div style="font-weight:800; color:#1E3A8A;">主單元　'
                + esc(pu) + '</div>' + suHtml + '</div>';
        }).join('');
    }
    ...
    function catalogCreateFolderNames() {
        ...
        try { teacherFolderNames().forEach(add); } catch (_e) {}
        if (window.MaterialComboStrategies && typeof window.MaterialComboStrategies.folderNames === 'function') {
            try { (window.MaterialComboStrategies.folderNames() || []).forEach(add); } catch (_e2) {}
        }
    ...
    function paintCreatePanel(wrap) {
        ...
        wrap.innerHTML = (
            ...
            + '<p style="color:#64748B; font-size:0.85rem; margin:0 0 10px 0;">這塊只產生目錄套餐，不准跟 Excel/JSON 套用、也不准跟 PDF 套用合併。出作業選這張卡、填主單元／次單元／標題／大題／次題／小題才收集。教材區窗口只顯示已有卡。</p>'
    ...
    function renderCreatePanel() {
        ...
        var meta = (window.FeatureTimeline && typeof window.FeatureTimeline.ensureMetaCatalog === 'function')
            ? window.FeatureTimeline.ensureMetaCatalog('', 'teacher', { force: false }).catch(function () {})
            : Promise.resolve();
        var strats = ensureLoaded();
        Promise.all([strats, meta]).then(function () {
    ...
    function renderPackTableHtml(ctx) {
        ...
        var html = rows.length
            ? ('<div class="range-pack-table"><div class="range-pack-table-inner">'
                + '<div class="range-pack-head range-pack-head--book"><div></div><div>主／次單元</div><div>頁碼</div><div>標題</div><div>大題</div><div>次題</div><div>小題</div><div>目錄文稿（可空）</div><div>刪</div></div>'
                + htmlRows
                + '</div></div>')
            : '';
    ...
    function bookRowInputsHtml(pathStr, idx, row, combo) {
        ...
        return (
            '<div>'
            + bookPickHtml(pathStr, pairId, pair, cat.unitPairs, '13-2', true)
            + '</div>'
            + '<div>'
            + bookPickHtml(pathStr, pageId, page, cat.pagesOf(primaryUnit, secondaryUnit), '頁碼', false)
            + '</div>'
            + '<div>'
            + bookPickHtml(pathStr, headingId, heading, cat.headingsOf(primaryUnit, secondaryUnit), '標題', false)
            + '</div>'
            + '<div>'
            + bookPickHtml(pathStr, majorId, major, cat.majorsOf(primaryUnit, secondaryUnit, heading, page), '大題', false)
            + '</div>'
            + '<div>'
            + bookPickHtml(pathStr, secId, secondary, cat.secondariesOf(primaryUnit, secondaryUnit, heading, major), '次題', false)
            + '</div>'
            + '<div>'
            + bookPickHtml(pathStr, minId, trim(row && row.minor), cat.minorsOf(primaryUnit, secondaryUnit, heading, major, secondary), '小題', false)
            + '</div>'
            + '<div>'
